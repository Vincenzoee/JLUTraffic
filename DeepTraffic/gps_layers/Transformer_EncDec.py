    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None):
        ...
        if self.conv_layers is not None:
            ...
            for attn_layer in self.attn_layers:
                x, attn = attn_layer(x, attn_mask=attn_mask, tau=tau, delta=delta)#这里x会输入到下一层
                attns.append(attn)
...
class TimeXerEncoder(nn.Module):
    ...
    def forward(self, x1, x2, attn_mask=None, tau=None, delta=None):
        ...
        if self.conv_layers is not None:
            ...
            for attn_layer in self.attn_layers:
                x1, attn = attn_layer(x1, x2, attn_mask=attn_mask, tau=tau, delta=delta)  # 这里x会输入到下一层
                attns.append(attn)
